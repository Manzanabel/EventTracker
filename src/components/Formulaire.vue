...
import type { Show, CreateShow } from '@/types/Show'
...
const createEmptyShow = (): CreateShow => {
  return {
    title: '',
    place: '',
    address: '',
...
const newShow = ref<CreateShow>(createEmptyShow())
const nbCrea = ref(0)
        ...
        <form @submit.prevent="addShow" class="space-y-4">
          <div v-for="input in eventInputs" :key="input.id" class="flex flex-col gap-1">
            ...
            <template v-else-if="input.type === 'select'">
              ...
              <select
                :id="input.id"
                v-model="newShow[input.id as keyof Show]"
            ...
            <template v-else-if="input.type === 'textarea'">
              ...
              <textarea
                :id="input.id"
                v-model="newShow[input.id as keyof Show] as string"
                :placeholder="input.placeholder"
                :required="input.required"
                rows="3"
                class="bg-slate-800 border border-slate-600 rounded-lg px-4 py-2 text-white placeholder-slate-500 focus:outline-none focus:border-fuchsia-400 transition-colors resize-none"
              />
            </template>
            ...
            <template v-else>
              ...
              <input
                ...
                :placeholder="input.placeholder"
                :required="input.required"
                :disabled="isDisabled(input.id)"
                @blur="input.id === 'title' ? maj() : null"
